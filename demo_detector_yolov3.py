...
class VideoScreenshot(object):
                ...
                try:
                    if os.path.exists(result):
                        with open(result) as namesFH:
                            ...
                            self.alt_names = [x.strip() for x in namesList]
                except TypeError:
                    pass
        except Exception:
            pass
    ...
    def show_frame(self):
        w,h = darknet.network_width(self.net_main), darknet.network_height(self.net_main)
        if self.write_video:
        # ID = 0
            out = cv2.VideoWriter('output_blend.avi',
                                  cv2.VideoWriter_fourcc('M', 'J', 'P', 'G'), 25,
                                  (darknet.network_width(self.net_main), darknet.network_height(self.net_main)))
        # Display frames in main program
        while True:
            if self.frame_detect is not None:
                frame = cv2.resize(self.frame_detect, (w*2, h*2))
                cv2.imshow('detect with yolo v3', frame)
                if self.write_video:
    ...
    def detect(self):
        while True:
            ...
            if self.frame_count % self.step_frame == 0:
                ...
                detections = darknet.detect_image(self.net_main, self.meta_main, self.darknet_image,
                                                  thresh=self.thresh)
                self.frame_count = 0
            end = time.time()
            fps_cur = 1/(end-start)
